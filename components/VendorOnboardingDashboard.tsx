  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        <ActionCard title="Vendor Registration Form" description="Public-facing onboarding form" to="/vendor/register" button="Open Form" color={VENDOR_COLORS.info} />
        <ActionCard title="Pending Approvals" description="Review and approve applications" to="/admin/vendors/pending-approvals" button="Review Queue" color={VENDOR_COLORS.warning} />
        <ActionCard title="Bulk Vendor Onboarding" description="Import multiple vendors at once" to="/admin/vendors/bulk-import" button="Bulk Import" color={VENDOR_COLORS.premium} />
        <ActionCard title="Status Management" description="Change vendor lifecycle states" to="/admin/vendors/status-management" button="Manage Status" color={VENDOR_COLORS.neutral} />
      </div>
            ...
            <tbody>
              {snap.applications.slice(0, 8).map((app) => (
                <tr key={app.applicationId} className="border-t border-slate-100">
                  ...
                  <td className="px-2 py-2 text-right"><Link className="text-blue-700 hover:underline" to={`/admin/vendors/review/${app.applicationId}`}>Review</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
