    ...
    <div>
        <div className="flex justify-between items-center mb-8">
            ...
            <div className="space-x-4">
                 <Link to="/client/rfi/create" className="bg-slate-800 text-white px-4 py-2 rounded flex inline-flex items-center space-x-2 hover:bg-slate-700">
                    <Plus size={16} /> <span>New RFI</span>
                 </Link>
                 <Link to="/client/rfq/create" className="bg-primary text-white px-4 py-2 rounded flex inline-flex items-center space-x-2 hover:bg-slate-800">
                ...
                <div className="space-y-3">
                    {rfis.map(rfi => (
                        <div key={rfi.id} className="p-3 bg-slate-50 rounded border border-slate-100 flex justify-between items-center">
                            ...
                            <Link to={`/client/rfi/${rfi.id}`} className="text-accent hover:underline text-sm font-medium">Manage</Link>
                        </div>
                    ))}
                ...
                <div className="space-y-3">
                    {rfqs.map(rfq => (
                        <div key={rfq.id} className="p-3 bg-slate-50 rounded border border-slate-100 flex justify-between items-center">
                            ...
                            <Link to={`/client/rfq/${rfq.id}`} className="text-accent hover:underline text-sm font-medium">Manage</Link>
                        </div>
                    ))}
