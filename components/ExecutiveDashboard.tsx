    ...
    <div className="space-y-8 min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-5">
        <EnhancedCard
          ...
          action={<button onClick={() => navigate('/admin/live-auctions')} className="text-xs text-blue-600 hover:text-blue-800 font-semibold">View All →</button>}
                    ...
                    <tr key={v.vendorId} className="border-t border-slate-100 hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent transition-colors duration-200">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="inline-flex gap-1">
                          ...
                          <button onClick={() => navigate('/contracts')} className="px-2.5 py-1 text-xs font-medium rounded-lg border-2 border-slate-200 hover:bg-slate-50 transition-colors">Contracts</button>
                        </div>
                      </td>
                    </tr>
                  );
                ...
                <tr key={`${row.auctionId}-${row.historical ? 'hist' : 'live'}`} className="border-t border-slate-100 hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent transition-colors duration-200">
                  ...
                  <td className="px-4 py-3 text-right">
                    {row.historical ? (
                      ...
                      <div className="inline-flex gap-1">
                        <button onClick={() => navigate(`/admin/analytics/auctions/${row.auctionId}`)} className="px-2.5 py-1 text-xs font-medium rounded-lg border-2 border-slate-200 hover:bg-slate-50 transition-colors inline-flex items-center gap-1">Analytics <ExternalLink size={12} /></button>
                        <button onClick={() => navigate(`/admin/auction-results/${row.auctionId}`)} className="px-2.5 py-1 text-xs font-medium rounded-lg border-2 border-slate-200 hover:bg-slate-50 transition-colors">Results</button>
                      </div>
                    )}
                  </td>
                </tr>
      ...
      <div className="flex items-center gap-4 bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
        ...
        <Link to="/admin/live-auctions" className="px-4 py-2 rounded-lg bg-blue-600 text-white font-semibold hover:bg-blue-700 transition-colors whitespace-nowrap">Open Live Auctions →</Link>
      </div>
    </div>
  );
}
