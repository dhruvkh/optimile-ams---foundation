    ...
    <div className="space-y-6">
            ...
            return (
              <div key={row.auction.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() => navigate(`/vendor/auction/${row.auction.id}/bid`)}
                    className="px-3 py-2 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700"
                  >
                    Join Auction
                  </button>
                  <button
                    onClick={() => navigate(`/admin/live-auctions/${row.auction.id}`)}
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          ...
          <QuickAccessCard title="Won Contracts" icon={<FolderOpen size={16} />} onClick={() => navigate('/contracts')} />
          <QuickAccessCard title="Pending Actions" icon={<Clock size={16} />} onClick={() => showToast({ type: 'warning', title: '2 actions pending' })} />
          <QuickAccessCard title="Queue Status" icon={<Bookmark size={16} />} onClick={() => navigate('/vendor/queue-status')} />
            ...
            return (
              <div key={auction.id} className="bg-white border border-slate-200 rounded-lg p-3 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button className="px-2 py-1 text-xs border rounded" onClick={() => navigate(`/vendor/auction/${auction.id}/bid`)}>Join Bidding</button>
                  <button
