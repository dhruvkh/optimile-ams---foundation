    ...
    return (
        <div className={`bg-white border rounded-lg p-4 flex items-center justify-between transition-all ${isRunning ? 'border-accent ring-1 ring-accent shadow-md' : 'border-slate-200'}`}>
            ...
            <div className="flex items-center space-x-2">
                ...
                {isRunning && (
                    <Link to={`/admin/lane/${lane.id}`} className="flex items-center space-x-1 px-3 py-1.5 rounded text-sm font-medium bg-red-50 text-red-600 hover:bg-red-100 border border-red-200">
                        <Activity size={14} /> <span>Live Monitor</span>
                    </Link>
                )}

                <Link to={`/vendor/${lane.id}`} target="_blank" className="p-2 text-slate-400 hover:text-primary hover:bg-slate-50 rounded" title="Vendor View">
